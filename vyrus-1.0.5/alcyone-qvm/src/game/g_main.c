...
// ACR cvars
vmCvar_t  g_blobEatLuci;
vmCvar_t  g_fueledjetpack;
vmCvar_t  g_jetmaxfuel;
vmCvar_t  g_devolve;
vmCvar_t  g_message;
vmCvar_t  g_cookiemsg;
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  // ACR cvars
  { &g_blobEatLuci, "g_blobEatLuci", "1", CVAR_ARCHIVE, 0, qfalse  },
  { &g_fueledjetpack, "g_fueledjetpack", "1", CVAR_ARCHIVE , 0, qfalse  },
  { &g_jetmaxfuel, "g_jetmaxfuel", "40", CVAR_ARCHIVE, 0, qfalse },
  { &g_devolve, "g_devolve", "2", CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },
  { &g_message, "g_message", "", CVAR_ARCHIVE, 0, qtrue },
  { &g_cookiemsg, "g_cookiemsg", "", 0, 0, qtrue },
  ...
  for( i = 0, cv = gameCvarTable; i < gameCvarTableSize; i++, cv++ )
  {
    if( cv->vmCvar )
    {
      ...
      if( cv->modificationCount != cv->vmCvar->modificationCount )
      {
        ...
        if( cv->trackChange )
        {
          // ACR   this gets annoying after a short SHORT while
          if( ( cv->cvarName != "g_message" ) || ( cv->cvarName != "g_cookiemsg" ) )
          {
...
void G_InitGame( int levelTime, int randomSeed, int restart )
{
  ...
  // ACR add default.cfg
  G_Printf( "executing default.cfg\n" );
  trap_SendConsoleCommand( EXEC_APPEND, "exec mapconfigs/default.cfg\n" );
  // ACR for !cookie ( i been fixin' it =] ) store custom id's to ram
  G_Printf("loading cookies >=)\n");
  trap_FS_FOpenFile( "cookies.txt", &msgFile, FS_READ );
  trap_FS_Read( msg, sizeof( msg ), msgFile );
  trap_FS_FCloseFile( msgFile );
  trap_Cvar_Set( "g_cookiemsg", msg );
  // ACR for !msg  read msg_temp, caus the damn cvar_norestart dont save line breaks in strings
  G_Printf("loading admin messages\n");
  trap_FS_FOpenFile( "msgpersist.txt", &msgFile, FS_READ );
  trap_FS_Read( msg2, sizeof( msg2 ), msgFile );
  trap_FS_FCloseFile( msgFile );
  trap_Cvar_Set( "g_message", msg2 );
...
int QDECL SortRanks( const void *a, const void *b )
{
  ...
  // then sort by score
  if( ca->pers.statscounters.mvp > cb->pers.statscounters.mvp )
    return -1;
  else if( ca->pers.statscounters.mvp < cb->pers.statscounters.mvp )
    return 1;
  else
    return 0;
}
...
void G_CalculateBuildPoints( void )
{
  ...
  if(!level.suddenDeath)
  {
    if(g_suddenDeath.integer || G_TimeTilSuddenDeath( ) <= 0 ) //Conditions to enter SD
    {
      ...
      {
        trap_SendServerCommand( -1, "cp \"^9Sudden Death!\"" );
    ...
    else 
    {
       ...
       {
         trap_SendServerCommand( -1, va("cp \"^9Sudden Death in %d seconds!\"", 
...
void CheckExitRules( void )
{
  ...
  while( 1 ) 
  {
   ...
   {
     amvp = cl->pers.statscounters.mvp;
     alienmvp = cl->pers.netname;
	 break;
   }
  ...
  while( 1 )
  {
   ...
   {
     hmvp = cl->pers.statscounters.mvp;
     humanmvp = cl->pers.netname;
	 break;
    } 
   if( i >= level.numConnectedClients )
      break; 
  ...
  if( g_timelimit.integer )
  {
    if( level.time - level.startTime >= g_timelimit.integer * 60000 )
    {
      ...
      trap_SendServerCommand( -1, va( "print \"^3Alien MVP^7: ^7%s ^3Score^7: ^1%d\n\"", alienmvp, amvp ) ); 
      trap_SendServerCommand( -1, va( "print \"^3Human MVP^7: ^7%s ^3Score^7: ^1%d\n\"", humanmvp, hmvp ) );
