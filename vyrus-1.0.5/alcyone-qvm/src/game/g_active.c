...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  int       aForward, aRight;
  int x, total;
  float percentNearBase, percentBuilding, percentCamping, timeCamping;
  ...
  while ( client->time100 >= 100 )
  {
    ...
    ===============
    */
     percentNearBase = ( ((float) client->pers.statscounters.timeinbase) / ((float) (client->pers.statscounters.timealive)) );
     percentBuilding = ( ((float) ( 8 * client->pers.statscounters.structsbuilt )) / ((float) (client->pers.statscounters.timealive)) );
     percentCamping = ( percentNearBase - percentBuilding );
     timeCamping = ( percentCamping * client->pers.statscounters.timealive );


     client->pers.statscounters.mvp = 
       ( ( client->pers.statscounters.assists * 10 ) +
         ( client->pers.statscounters.kills * 20 ) +
         ( client->pers.statscounters.structskilled * 5 ) +
         ( client->pers.statscounters.structsbuilt * 5 ) -
         ( client->pers.statscounters.deaths * 0.5 ) -
       ( timeCamping * 0.25 ) );

    if( client->pers.statscounters.mvp < -999 )
    {
     client->pers.statscounters.mvp = -999;
     if( ( level.time % 600000 ) == 0 )
      trap_SendServerCommand( ent-g_entities, va( "print \"^3you're ^1REALLY ^3sucking!!!\n\"" ) );
    }
  ...
  while( client->time1000 >= 1000 )
  {
    ...
    if( g_fueledjetpack.integer == 1 ) 
    {
      ...
      {
        if( client->jetpackfuel > ( g_jetmaxfuel.integer - 5.0f ) && client->jetpackfuel <= g_jetmaxfuel.integer  ) 
        {
        ...
        else if( client->jetpackfuel < g_jetmaxfuel.integer )
        { 
        client->jetpackfuel = client->jetpackfuel + JETPACK_REGEN_RATE;
        }
      }
