...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"denyvote", G_admin_denyvote, "d",
      "take away a player's voting rights or give them back",
      "[^3name|slot#^7]"
    },

    {
     "deny", G_admin_denyweapon, "d",
    ...
    {"devolve", G_admin_devolve, "k",
      "^3!devolve^7: change the state of devolve",
      "^3useage^7: !devolve ^1[^7#^1]"
    },
    ...
    {"msg", G_admin_msg, "?",
      "^3shows an admin message\n!msg ^7<this will record>",
      "^3!msg"
    },
...
qboolean G_admin_L0(gentity_t *ent, int skiparg ){
  ...
  if (id >= 0 && id < level.maxclients)
  {
    ...
    if( G_admin_level( vic ) > 2 )
    {
      ADMP( "^3!L0:^7 intended victim is above level 2\n" );
      return qfalse;
    }
  }
    ...
    && g_admin_admins[ id - MAX_CLIENTS ] )
  {
    if( g_admin_admins[ id - MAX_CLIENTS ]->level > 2 ) 
    {
      ADMP( "^3!L0:^7 intended victim is above level 2\n" );
      return qfalse;
    }
...
qboolean G_admin_L2(gentity_t *ent, int skiparg ){
  ...
  if( G_admin_level( &g_entities[ pids[ 0 ] ] ) > 2 )
  {
    ADMP( "^3!L2: ^7Sorry, but that person is already higher than level 2.\n" );
    return qfalse;
  }
...
qboolean G_cookie( gentity_t *ent, int skiparg )
{
    int pids[ MAX_CLIENTS ], sl = 0, i = 0;
    char name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
    char cookietext[ MAX_STRING_CHARS ] = { "a cookie!" };
    ...
    if( G_SayArgc() < 2 + skiparg )
    {
        ADMP( "^3!cookie: ^7usage: !cookie [name]\n" );
        return qfalse;
    }
    if( G_ClientNumbersFromString( name, pids ) != 1 )
    {
    ...
    trap_Cvar_VariableStringBuffer( "g_cookiemsg", msg, sizeof( msg ) );
    
    sl = findint( vic->client->pers.adminName, msg );
    ...
    AP( va( "print \"%s^7 gave %s^7 %s\n\"", ( ent ) ? ent->client->pers.netname : "console", vic->client->pers.netname, cookietext ) );

    return qtrue;        
}


 qboolean G_ragequit( gentity_t *ent, int skiparg )
 {  
   AP( va( "print \"%s^7 has just ^1RAGEQUIT!\n\"",
           ent->client->pers.netname) );
  trap_DropClient( ent->client - level.clients, "ragequit" );
 ...
 qboolean G_admin_kill( gentity_t *ent, int skiparg )
 {
   ...
   if( !admin_higher( ent, &g_entities[ pids[ 0 ] ] ) )
   {
     ADMP( "^3!kill: ^7sorry, but your intended victim has a higher "
         " admin level than you\n" );
     return qfalse;
   }
   vic = &g_entities[ pids[ 0 ] ];
   
    if( vic->client->ps.stats[ STAT_STATE ] & SS_INFESTING )
     return qfalse;

 		G_Damage( vic, vic, vic, NULL, NULL, 10000, 0, MOD_SUICIDE );
       player_die( vic, vic, vic, 100000, MOD_SUICIDE );
       G_Damage( vic, vic, vic, NULL, NULL, 10000, 0, MOD_SUICIDE );
   return qtrue;
 }

qboolean G_admin_devolve( gentity_t *ent, int skiparg )
{
 ...
 var = atoi( arg );
  ...
  if( G_SayArgc() < 2 + skiparg )
  {
    ADMP( "^3!devolve^7: change the state of devolve\n" );
    ADMP( "^3useage^7: !devolve ^1[^7#^1]\n" );
    return qfalse;
  }

  if( ( var >= 0 ) && ( var <= 3 ) )
   trap_Cvar_Set( "g_devolve", va( "%d", var ) );

  if( var == 0 )
   AP( "cp \"^3!devolve: ^7Devolve is now ^1OFF\n\"" );
  else if( var == 1 )
   AP( "cp \"^3!devolve: ^7Devolve is now ^1ON\n\"" );
  else if( var == 2 )
   AP( "cp \"^3!devolve: ^7Devolve timer ^1ON\n\"" );
  else if( var == 3 )
   AP( "cp \"^3!devolve: ^7Devolve range ^1ON\n\"" );
  else
   {
    ADMP( "^3!devolve^7: change the state of devolve\n" );
    ADMP( "^3useage^7: !devolve ^1[^7#^1]\n" );
 ...
 qboolean G_admin_denyweapon( gentity_t *ent, int skiparg )
  {
    ...
    CPx( pids[ 0 ], va( "cp \"^1You've %s your %s rights\"",
      ( !Q_stricmp( cmd, "deny" ) ) ? "lost" : "regained",
      realname ) );
    AP( va(
      "print \"^3!%s: ^7%s rights for ^7%s^7 %s by %s\n\"",
      cmd, realname,
      vic->client->pers.netname,
      ( !Q_stricmp( cmd, "deny" ) ) ? "revoked" : "restored",
      ( ent ) ? ent->client->pers.netname : "console" ) );
 ...
 qboolean G_admin_forcespec( gentity_t *ent, int skiparg )
  {
    int pids[ MAX_CLIENTS ];
    char name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ], team[ 7 ];
    char teamdesc[ 32 ] = {"spectate"};
    int minargc;
    gentity_t *vic;
    pTeam_t teamnum = PTE_NONE;
    ...
    if( G_SayArgc() < minargc )
    {
      ADMP( "^3!forceteam: ^7usage: !forceteam [name|slot#] [h|a|s]\n" );
      return qfalse;
    }
    G_SayArgv( 1 + skiparg, name, sizeof( name ) );
    G_SayArgv( 2 + skiparg, team, sizeof( team ) );
   ...
   if( !admin_higher( ent, &g_entities[ pids[ 0 ] ] ) )
    {
      ADMP( "^3!forceteam: ^7sorry, but your intended victim has a higher admin level than you\n" );
      return qfalse;
    }
   ...
   switch( team[ 0 ] )
   {
    ...
    case 's':
      ...
      break;
    default:
      ADMP( va( "^3!forceteam: ^7what the hell is %c\n", team[ 0 ] ) );
      return qfalse;
   }

   if( ( vic->client->pers.forced == qtrue ) && ( vic->client->pers.teamSelection == teamnum ) )
   {
    ADMP( "^3!forceteam: ^7player already forced to that team\n" );
  	return qfalse;
   }


  //tell everyone player has been forced & tell player
  AP( va( "print \"^3!forceteam: ^7%s^7 has forced ^7%s^7 to %s\n\"", ( ent ) ? ent->client->pers.netname : "console", vic->client->pers.netname, teamdesc ) );
  CPx( pids[ 0 ], va( "cp \"^1you have been forced to %s\n\"", teamdesc ) );
  //move to forced team
  G_ChangeTeam( vic, teamnum );
  //tell g_cmds that they cannot join teams
  vic->client->pers.forced = qtrue;
 ...
 qboolean G_admin_unforcespec( gentity_t *ent, int skiparg )
  {
    ...
    if( G_SayArgc() < minargc )
    {
      ADMP( "^3!unforceteam: ^7usage: !unforceteam [name|slot#]\n" );
      return qfalse;
    }
    G_SayArgv( 1 + skiparg, name, sizeof( name ) );
    ...
    {
      G_MatchOnePlayer( pids, err, sizeof( err ) );
      ADMP( va( "^3!unforceteam: ^7%s\n", err ) );
      return qfalse;
    }
  
    vic = &g_entities[ pids[ 0 ] ];
  
   if ( vic->client->pers.forced == qfalse )
   {
   ADMP( "^3!unforceteam: ^7player is not forced\n" );
  ...
  vic->client->pers.forced = qfalse;
  //tell them it was removed
  CPx( pids[ 0 ], "cp \"^1you are no longer forced to a team\"" );
  //tell everyone that it was removed
  AP( va( "print \"^3!unforceteam: ^7%s^7 has unforced ^7%s\n\"", ( ent ) ? ent->client->pers.netname : "console", vic->client->pers.netname ) );
  	return qtrue;
...
qboolean G_admin_denyvote( gentity_t *ent, int skiparg )
 {
   ...
   vic = &g_entities[ pids[ 0 ] ];
   if( vic->client->pers.voteCount == -1 )
   {    
     vic->client->pers.voteCount = 5;
     CPx( pids[ 0 ], "cp \"^1You've regained your voting rights\"" );
     AP( va(
       "print \"^3!allowbuild: ^7voting rights for ^7%s^7 restored by %s\n\"",
       vic->client->pers.netname,
       ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
   }
   else
   {
     vic->client->pers.voteCount = -1;
     CPx( pids[ 0 ], "cp \"^1You've lost your voting rights\"" );
     AP( va(
       "print \"^3!denybuild: ^7voting rights for ^7%s^7 revoked by ^7%s\n\"",
       vic->client->pers.netname,
       ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
   }
   ClientUserinfoChanged( pids[ 0 ] );
...
qboolean G_admin_msg( gentity_t *ent, int skiparg )
{
    fileHandle_t msgfile;
    char msg[ MAX_STRING_CHARS ] = { "" };
    int i = 0, msgs = 0;
  
    // if no parameters, print out the local message
    if( G_SayArgc() < 2 + skiparg )
    {
        ADMP( g_message.string );
        return qtrue;
    }
    // for not maxing out cvars:
    if( getsize( G_SayConcatArgs( 1 + skiparg ) ) > ( MAX_CVAR_VALUE_STRING / 5 ) )
    {
        ADMP( va("^1message size too long, please keep messages under %i characters.\n", ( MAX_CVAR_VALUE_STRING / 5 ) ) );
        return qfalse;
    }
  
    // count the messages in g_message
    for( i = 0; i < getsize(g_message.string); i++ )
    {
        if( g_message.string[i] == '\n' )
            msgs++;
    }
    
    // find value message starts at excluding first line if there are 10 msgs
    if( msgs > 4 )
    {
        for( i = 0; i < getsize(g_message.string); i++ )
        {
            ...
            {
                i++;
                strncpy( msg, g_message.string+i, getsize(g_message.string+i) );
                break;
            }
        }
    ...
    {
        trap_Cvar_VariableStringBuffer( "g_message", msg, sizeof( msg ) );
    }

    // set the cvar to our new message (   ( old msg - first ) + admin name + : + new + linebreak   )
    trap_Cvar_Set( "g_message", va( "%s  %s^7: %s\n", msg, ent->client->pers.adminName, G_SayConcatArgs( 1 + skiparg ) ) );
    ADMP(va("%s\n",g_message.string));

    // create file if one does not exist
    if( trap_FS_FOpenFile( "msglog.txt", &msgfile, FS_READ ) <= 0 )
    {
        trap_FS_FCloseFile( msgfile );
        trap_FS_FOpenFile( "msglog.txt", &msgfile, FS_WRITE );
        trap_FS_FCloseFile( msgfile );
    }
    
    // send the new message to our log file for record    
    trap_FS_FOpenFile( "msglog.txt", &msgfile, FS_APPEND );
    trap_FS_Write( ent->client->pers.adminName, strlen(ent->client->pers.adminName), msgfile );
    trap_FS_Write( "^7: ", strlen("^7: "), msgfile );
    trap_FS_Write( G_SayConcatArgs( 1 + skiparg ), strlen(G_SayConcatArgs( 1 + skiparg )), msgfile);
    trap_FS_Write( "\n", strlen("\n"), msgfile );
    trap_FS_FCloseFile( msgfile );
    
    // send the entire current message log to file for persistence
    trap_FS_FOpenFile( "msgpersist.txt", &msgfile, FS_WRITE );
    trap_FS_Write( va( "%s  %s^7: %s\n", msg, ent->client->pers.adminName, G_SayConcatArgs( 1 + skiparg ) ), strlen(va( "%s  %s^7: %s\n", msg, ent->client->pers.adminName, G_SayConcatArgs( 1 + skiparg ) )), msgfile );
    trap_FS_FCloseFile( msgfile );

    
    ADMP( "^1message recorded^7\n" );
    return qtrue;
}
