...
void Cmd_Kill_f( gentity_t *ent )
{
  ...
  else
  {
    ...
    {
      trap_SendServerCommand( ent-g_entities, "print \"You will suicide in 0 second\n\"" );
      ent->suicideTime = level.time + 10;
    }
...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  if( ent->client->pers.muted )
  {
   ...
   if( ent->client->pers.voteCount == -1 )
   {
...
void Cmd_Class_f( gentity_t *ent )
{
      ...
      !( ent->client->ps.stats[ STAT_STATE ] & SS_HOVELING ) )
  {
    ...
    if( ent->client->pers.classSelection != PCL_NONE )
    {
      ...
      //devolve timer
      if( ( g_devolve.integer == 2 ) &&
          ( ent->client->sess.sessionTeam != TEAM_SPECTATOR ) &&
          ( currentClass == PCL_ALIEN_LEVEL0 ) &&
          ( ent->client->ps.stats[ STAT_MISC ] > 0 ) )
      {
        trap_SendServerCommand( ent-g_entities,
            va( "print \"You cannot evolve until devolve timer expires\n\"" ) );
        return;
      }
...
void Cmd_Buy_f( gentity_t *ent )
{
  ...
  else if( upgrade != UP_NONE )
  {
    ...
    //jetpack fuel full when added
    if( upgrade == UP_JETPACK )
     ent->client->jetpackfuel = g_jetmaxfuel.integer;
...
void Cmd_Build_f( gentity_t *ent )
{
  ...
  // ACR - prevent general gayness that has cropped up
  if( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_A_SPAWN ) && ent->client->pers.teamSelection == PTE_HUMANS && ( buildable == BA_H_MGTURRET || buildable == BA_H_TESLAGEN ) )
  {
      trap_SendServerCommand( ent-g_entities, "print \"You may not build near enemy spawns\n\"" );
      return;
  }
  if( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_H_SPAWN ) && ent->client->pers.teamSelection == PTE_ALIENS && ( buildable == BA_A_TRAPPER || buildable == BA_A_HIVE || buildable == BA_A_ACIDTUBE ) )
  {
      trap_SendServerCommand( ent-g_entities, "print \"You may not build near enemy spawns\n\"" );
      return;
  }
  if( ent->client->pers.teamSelection == PTE_ALIENS && buildable == BA_A_SPAWN && ( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_H_TESLAGEN ) || G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_H_MGTURRET ) ) )
  {
      trap_SendServerCommand( ent-g_entities, "print \"This area is not safe\n\"" );
      return;
  }
  if( ent->client->pers.teamSelection == PTE_HUMANS && buildable == BA_H_SPAWN && ( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_A_TRAPPER ) || G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_A_HIVE ) || G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_A_ACIDTUBE ) ) )
  {
      trap_SendServerCommand( ent-g_entities, "print \"This area is not safe\n\"" );
      return;
  }
...
void Cmd_devolve_f( gentity_t *ent )
{
       ...
       {
        ADMP( "^3!devolve: ^7sorry, aliens only \n" );
       }
 	...
 	if( ( ent->client->ps.stats[ STAT_STATE ] & SS_WALLCLIMBING ) ||
           ( ent->client->ps.stats[ STAT_STATE ] & SS_WALLCLIMBINGCEILING ) )
         trap_SendServerCommand( ent-g_entities,"print \"You cannot devolve while wallwalking\n\"" );
 	else
 	{
 	 if( currentClass == PCL_ALIEN_BUILDER0 || 
             ...
             currentClass == PCL_ALIEN_LEVEL0 )
 	 {
 	  trap_SendServerCommand( ent-g_entities, "print \"You cannot devolve from your current class\n\"" );
          cost = 0;
         }
 	 else
         {
 	  if(currentClass == PCL_ALIEN_LEVEL1)
 	   cost = 1;
 	  if(currentClass == PCL_ALIEN_LEVEL2 || currentClass == PCL_ALIEN_LEVEL1_UPG)
 	   cost = 2;
 	  if(currentClass == PCL_ALIEN_LEVEL3 || currentClass == PCL_ALIEN_LEVEL2_UPG)
 	   cost = 3;
 	  if(currentClass == PCL_ALIEN_LEVEL3_UPG)
 	   cost = 4;
 	  if(currentClass == PCL_ALIEN_LEVEL4)
 	   cost = 5;
 	  if(ent->client->pers.credit < 2)
 	   trap_SendServerCommand( ent-g_entities, "print \"Not Enough Evolve Points, Costs 2\n\"" );   
 	  else if( ( G_BuildableRange( ent->client->ps.origin, 900, BA_A_OVERMIND ) &&
                   ( g_devolve.integer == 3 ) ) ||
                   ( g_devolve.integer == 2 ) ||
                   ( g_devolve.integer == 1 ) )
 	  {
 	   ...
 	   if(evo < 0)
 	    evo = 0;
 	   ent->client->pers.evolveHealthFraction = ( (float)ent->client->ps.stats[ STAT_HEALTH ] / 
                (float)BG_FindHealthForClass( PCL_ALIEN_LEVEL0 ) );
           ...
           //remove credit
 	   G_AddCreditToClient( ent->client, +(short)evo, qtrue );
           ...
           //devolve conditions
           if( g_devolve.integer == 2 )
            ent->client->ps.stats[ STAT_MISC ] = 10000;
 	  }
         else 
          if( g_devolve.integer == 3 )
           trap_SendServerCommand( ent-g_entities, "print \"You may not devolve outside of your base\n\"" );
      }}}
}
...
commands_t cmds[ ] = {
  ...
  { "devolve", CMD_ALIEN|CMD_LIVING, Cmd_devolve_f }
};
...
void G_PrivateMessage( gentity_t *ent )
{
  ...
  for( i=0; i < matches; i++ )
  {
	...
	for( x = 0; x < level.maxclients; x++ )
  {
    if( G_admin_level( &g_entities[ x ]) == 999 )
    {
	prefix = "[PM]";
      //G_SayTo( ent, &g_entities[ x ], SAY_TEAM, color, name, msg, prefix ); 
	  clinum = &g_entities[ x ] - g_entities;
		trap_SendServerCommand( clinum, va( 
      "chat \"^3[PM]^2[%s] to [%s]^1: ^7%s\"",
      ent->client->pers.netname,name,msg) );	  
    } 
  }
...
// ACR - commands for mah use =]

int getsize( char *string ) // I do NOT trust sizeof() to do what I need it to do...
{
    int i = 0;
    while( string[i] != '\0' )
        i++;
...
qboolean stringcpr( char *vicname, char *name )  // compare two strings for exact character match
{
    int i = 0, vicname_size = 0, name_size = 0;
    vicname_size = getsize(vicname);
    name_size = getsize(name);

    if( vicname_size != name_size )
        return qfalse;
    while( i < getsize(name) )
    {
        if( name[i] != vicname[i] )
            return qfalse;
        i++;
...
qboolean find( char *this, char *inthis )
{
    int i = 0, a = 0, this_size, inthis_size;
    this_size = getsize(this);
    inthis_size = getsize(inthis);
    while( inthis[i] != '\0' || inthis[i+this_size] != '\0' )
    {
        a = 0;
        while( a < this_size+2 ) // had to put it as SOMETHING now didn't I
        {
            if( this[a] == '\0' )
                return qtrue;
            if( this[a] != inthis[i+a] )
                break;
            a++;
        }
        i++;
...
int findint( char *this, char *inthis )  // this one's specific for !cookie, so I can get a return int
{                                        // although now i think of it IMA USE IT FOR MORE! =]
    int i = 0, a = 0, this_size, inthis_size;
    this_size = getsize(this);
    inthis_size = getsize(inthis);
    while( inthis[i] != '\0' || inthis[i+this_size] != '\0' )
...
// returns the int of the end char of specified # of msgs unless less msgs total
int msgend( char msg[], int end )
{
    int i=0;
    int lines=0;
    
    //  find the number of messages in the file
    for(i=0;i < getsize(msg);i++)
    {
       if( msg[i] == '\n' )
           lines++;
       if( lines == end )
