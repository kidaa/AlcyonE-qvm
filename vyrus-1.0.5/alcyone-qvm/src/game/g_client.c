...
void ClientBegin( int clientNum )
{
    ...
    if( !Q_stricmp( ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" ) )
    {
      trap_SendServerCommand( client->ps.clientNum, va( "print \"^1Your client is out of date. Updating your client will allow you to "
        "become an admin on servers and download maps much more quickly. Please replace your client executable with the one "
         "at ^2http://vyrus.zzl.org/^1 and reconnect. \n\"" ) );
    }
  }
  ...
  //trap_SendServerCommand( client->ps.clientNum, va( "print \"^6update 10/1/11\n^1* ^7mvp score updated\n^1* ^7fixed !allow and !deny\n^1* ^7re implemented lcannon overcharge\n\"" ) );
  //trap_SendServerCommand( client->ps.clientNum, va( "print \"^6update 2/2/12\n^1* ^7building around spawns eliminated\n^1* ^7wait for re-boost removed\n^1* ^7!msg complete\n^1* ^7!cookie complete\n^1* ^7light armor and helmet protection 2x\n\"" ) );
  trap_SendServerCommand( client->ps.clientNum, "print \"^1!^7info mods for information on the server's mods\n\"" );
  if( ent->client->pers.adminLevel >= MINIMUM_MESSAGE_LEVEL )
      trap_SendServerCommand( client->ps.clientNum, va( "print \"^3!msg:\n%s\n\"", g_message.string ) );
