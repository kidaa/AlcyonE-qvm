...
void ABooster_Touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
  ...
  // ACR  - allow for re-boost even though u have poizon
...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin )
{
  ...
  if(  ( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_A_SPAWN ) && ent->client->pers.teamSelection == PTE_HUMANS && ( buildable == BA_H_MGTURRET || buildable == BA_H_TESLAGEN ) ) || ( G_BuildableRange( ent->client->ps.origin, SPAWN_SAFE_RANGE, BA_H_SPAWN ) && ent->client->pers.teamSelection == PTE_ALIENS && ( buildable == BA_A_TRAPPER || buildable == BA_A_HIVE || buildable == BA_A_ACIDTUBE ) )  )
      reason = IBE_ESPAWN;
