...
void G_ExplodeMissile( gentity_t *ent )
{
 ...
 Explode a missile without an splashdamage
 ================
 */
 void G_deletemissile( gentity_t *ent, int splash )
 {
   vec3_t    dir;
   vec3_t    origin;
   ...
   if( ent->s.weapon != WP_LOCKBLOB_LAUNCHER &&
       ent->s.weapon != WP_FLAMER )
     //G_AddEvent( ent, EV_MISSILE_MISS, DirToByte( dir ) );      // i think this is what makes all teh splashyz ;D
   ...
   // :D win!
   trap_LinkEntity( ent );   // deletes the ent
 }
...
void G_GuideMissile( gentity_t *ent ) 
{
  ...
  {       // if the owner is dead
          // normal rocket again for 2 seconds
  ...
  {          // owner is alive
   ...
   VectorScale (dir,VectorLength(ent->s.pos.trDelta),ent->s.pos.trDelta);
   ...
   ent->nextthink = 50; // Think again in 50ms
  }
} 
...
void G_RunMissile( gentity_t *ent )
{
  ...
  trap_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask );
   ...
   if( g_blobEatLuci.integer )
   {
    ...
    {
      luci = &g_entities[ i ]; 
      if( luci->classname == "lcannon" )
      {
        for( j = 0; j < MAX_GENTITIES; j++ )
        {
         advg = &g_entities[ j ];
         if( advg->classname == "bounceball" )
         {
           if( Distance( luci->r.currentOrigin, advg->r.currentOrigin ) < ( HIVE_RANGE / 12 ) )
           {
             //luci->damage = luci->damage;
             luci->splashDamage = luci->splashDamage * 0.75;
             advg->methodOfDeath = luci->methodOfDeath;
             //luci->splashRadius = luci->splashRadius;
             G_MissileImpact( luci, &tr );
                            luci->r.ownerNum = advg->r.ownerNum;
                            luci->parent = advg->parent;
             if( ent->s.eType != ET_MISSILE )
                return;   // exploded
           }
         }
         if( advg->classname == "slowblob" )
         {
           if( Distance( luci->r.currentOrigin, advg->r.currentOrigin ) < ( HIVE_RANGE / 10 ) )
           {
                        if( Distance( luci->parent->r.currentOrigin, luci->r.currentOrigin ) < ( HIVE_RANGE / 4 ) )
                        {
                            G_deletemissile(luci, 0);
                            G_ExplodeMissile(advg);
                            G_TempEntity( advg->r.currentOrigin, EV_PLAYER_TELEPORT_OUT );
                        }
                        else
                        {
                        
                            G_deletemissile(luci, 1);
                            G_ExplodeMissile(advg);
                            G_TempEntity( advg->r.currentOrigin, EV_PLAYER_TELEPORT_OUT );
                        }                 
